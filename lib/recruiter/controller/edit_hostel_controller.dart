...
class EditHostelController extends GetxController {
  ...
  addData(HostelAddModel model) {
    List<RoomType> roomTypeData =
        model.roomType!.map((e) => RoomType.fromJson(e)).toList();
    description.text = model.description!;
    hostelAddress.text = model.location!;
    hostelname.text = model.name!;
    hostellocalAddress.text = model.address!;
    avalBed.text = model.avalaibleBed!;
    avalRoom.text = model.avalaibleRoom!;
    _selectedGender.value = model.gender!;
    for (int i = 0; i < roomTypeController.length; i++) {
      for (var d in roomTypeData) {
        if (d.type == i) {
          roomTypeController[i].text = d.price!;
          _isRoomSelected[i] = true;
          // _selectedRoomType.add(d);
        }
      }
    }
    List<int> animtsData = model.amenities!.map((e) => e).toList();
    for (int i = 0; i < aminties.length; i++) {
      for (int x = 0; x < animtsData.length; x++) {
        if (x == i) {
          _selectedAmenitiesType.add(animtsData[x]);
        }
      }
    }
    phone.text = model.phone!;
    poilicy.text = model.policies!;
    currentLatLng.value = LatLng(model.lat!, model.long!);
    email.text = model.email!;
    docId.text = model.docId!;
